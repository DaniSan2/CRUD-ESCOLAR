...
import { AlertCircle, Plus, Edit2, Trash2, ArrowRight, MSquare } from "lucide-react";
import axios from "axios";

// url de la api dani, yo uso el puerto 5000 pero si tienes broncas se puede cambiar
const API_URL = "http://127.0.0.1:5000/materias"; // asegurate que corra bien este es muy importante

const AppMaterias = () => { // lista de carreras
  const [alumnos, setAlumnos] = useState([]); // lista de alumnos
  const [grupos, setGrupos] = useState([]);
  const [carreras, setCarreras] = useState([]);
  const [materias, setMaterias] = useState([]);
  const [profesores, setProfesores] = useState([]); // lista de carreras // lista de carreras // lista de carreras
  const [error, setError] = useState(null); // para manejar errores
  const [showMateriaForm, setShowMateriaForm] = useState(false); // muestra u oculta el formulario
  const [selectedMateria, setSelectedMateria] = useState(null); // guarda la carrera seleccionada para editar
  ...
  const handleSaveMateria = async () => {
    try {
      if (selectedMateria) {
        await axios.put(`${API_URL}/${selectedMateria.idMateria}`, newMateria, {
      ...
      fetchMaterias(); // refresca la lista de alumnos
      setShowMateriaForm(false); // cierra el form
      setSelectedMateria(null); // limpia la seleccion
      setNewMateria({ nombreMateria: "", creditos: "", semestre: "", idCarrera:"" });
    } catch (err) {
  ...
  const handleDeleteMateria= async (idMateria) => {
    try {
      await axios.delete(`${API_URL}/${idMateria}`);
      fetchMaterias(); // refresca la lista de alumnos
    } catch (err) {
  ...
  const handleEditMateria = (materia) => {
    setSelectedMateria(materia); // selecciona la carrera para editar
    setNewMateria({
        ...
        <table className="w-full table-auto border-collapse border border-gray-300">
          <thead className="bg-blue-500 text-white">
            <tr>
              ...
              <th className="p-4 text-left">creditos</th>
          ...
          <tbody>
            ...
            {materias.map((materia) => (
              <tr key={materia.idMateria} className="border-b hover:bg-blue-100 transition">
                <td className="p-4">{materia.nombreMateria}</td>
                <td className="p-4">{materia.creditos}</td>
      ...
      {showMateriaForm && (
        <div className="fixed inset-0 bg-gray-800 bg-opacity-50 flex justify-center items-center">
          <div className="bg-white p-6 rounded-lg shadow-lg w-1/3">
            <h3 className="text-2xl font-bold mb-4">{selectedMateria ? "Editar Materia" : "Nueva Materia"}</h3>
            <input
              type="text"
              placeholder="Nombre de la Materia"
              value={newMateria.nombreMateria}
              onChange={(e) => setNewMateria({ ...newMateria, nombreMateria: e.target.value })}
            ...
            <input
              ...
              onChange={(e) => setNewMateria({ ...newMateria, creditos: e.target.value })}
              className="w-full p-2 border border-gray-300 rounded mb-4"
            />
            <input
              type="number"
            ...
            <input
              type="number"
              placeholder="IdCarrera"
              value={newMateria.idCarrera}
              onChange={(e) => setNewMateria({ ...newMateria, idCarrera: e.target.value })}
              className="w-full p-2 border border-gray-300 rounded mb-4"
            />
            <div className="flex justify-end gap-4">
              <button onClick={handleSaveMateria} className="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">
                Guardar
              </button>
              <button
                onClick={() => {
